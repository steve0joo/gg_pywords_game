...
SCORE_FILE = 'word_game_score.csv'
GOOD_SOUND = 'assets/good.wav'
BAD_SOUND = 'assets/bad.wav'
...
def gameRun(words):
    ...
    for trial in range(LIFE):
        answer = askWord(trial, words[trial])
        correct = correct + wordLoad(answer, words[trial])
...
def saveScore(score_file, execution_time, correct):
...
def main():
    ...
    words = loadWords(WORD_FILE)

    start_time = time.perf_counter()
    correct = gameRun(words)
    execution_time = time.perf_counter() - start_time

    scorePrint(correct)
    print(f"게임 걸린시간: {execution_time:.2f} 초, 맞춘 개수 : {correct}개")

    saveScore(SCORE_FILE, execution_time, correct)
